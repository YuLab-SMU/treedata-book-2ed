...
# ggtree for Other Tree-like Objects {#sec-chapter9}
...
The `r Biocpkg("ggtree")` package supports `phylo4d` object and all the associated data stored in the `phylo4d` object can be used directly to annotate the tree (Figure @fig-fp4d).
...
#| label: fig-fp4d
#| fig-cap: |
#|   **Visualizing `phylo4d` data using ggtree.**
#| fig-subcap:
#|   - "Reproduce the output of the plot() method provided in the phylobase package"
#|   - "Visualize trait data as a heatmap, which is not supported in phylobase"
#| fig-scap: "(ref:fp4dscap)"
#| out-width: "90%"
#| fig-show: hold
#| layout-ncol: 2
library(phylobase)
...
### The phyloseq object {#sec-phyloseq}
...
#| fig-scap: "(ref:reproducephyloseqscap)"
#| out-extra: ''
#| warning: false
#| out-width: '100%'
library(phyloseq)
...
Figure @fig-reproducephyloseq reproduces the output of the `phyloseq::plot_tree()` function. Users of `r Biocpkg("phyloseq")` will find `r Biocpkg("ggtree")` useful for visualizing microbiome data and for further annotation since `r Biocpkg("ggtree")` supports high-level annotation using the grammar of graphics and can add tree data layers that are not available in `r Biocpkg("phyloseq")`.
...
## ggtree for Dendrograms {#sec-dendrogram}
...
```{r}
#| echo: false
#| label: tbl-tree-objects
#| tbl-cap: |
#|   Supported tree-like objects in `r Biocpkg("ggtree")`.
tree_objects <- data.frame(
  Object = c(
    "phylo",
    "multiPhylo",
    "phylo4",
...
## ggtree for Tree Graph {#sec-igraph}

The tree graph (as  an `igraph` object) can be converted to a `phylo` object using `as.phylo()` method provided in the `r Biocpkg("treeio")` package (Table @tbl-tree-objects). The `r Biocpkg("ggtree")` supports directly visualizing tree graph as demonstrated in Figure @fig-treeGraph. Note that currently not all `igraph` objects can be supported by `r Biocpkg("ggtree")`. Currently, it can only be supported when it is a tree graph\index{tree graph}.
...
#| label: fig-gni2014
#| fig-cap: |
#|   **Visualizing data in any hierarchical structure.** Hierarchical data represented as nodes connected by edges can be converted to a `phylo` object and visualized by `r Biocpkg("ggtree")` to explore their relationships or other properties that are associated with the relationships\index{dendrogram}.
#| fig-scap: "(ref:gni2014scap)"
#| out-width: '100%'
library(treeio)
library(ggplot2)
library(ggtree)
