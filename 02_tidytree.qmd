...
# Manipulating Tree with Data {#sec-chapter2}
...
## Manipulating Tree Data Using Tidy Interface {#sec-tidytree}
...
### Access related nodes {#sec-accesor-tidytree}
...
### Combining tree data {#sec-merge-tree}
...
#| fig-scap: "Correlation of dN/dS vs. substitution rate"
#| out-extra: ''
#| out-width: "100%"
library(dplyr)
df <- merged_tree %>% 
...
The output is illustrated in Figure @fig-correlations. We can then test the association of these node/branch-specific data using Pearson correlation, which in this case showed that *d~N~* and *d~S~*, but not *d~N~/d~S~*\index{d\textsubscript{N}/d\textsubscript{S}}  are significantly (*p*-values) associated with *rate*.
...
### Linking external data to phylogeny {#sec-link-external-data}
...
with `r Biocpkg("ggtree")` (Figure @fig-groupOTU).
...
## Rerooting tree {#sec-reroot-treeio}
...
Then we can reroot the tree with the associated data mapping to the branches and nodes correctly as demonstrated in Figure @fig-reroot. The figure was visualized using `r Biocpkg("ggtree")` (see also Chapters [4](#chapter4) and [5](#chapter5)).
...
#| label: fig-reroot
#| fig-cap: |
#|   **Reroot a phylogenetic tree with associated data.**
#| fig-subcap:
#|   - "Original tree"
#|   - "Rerooted tree, with associated data mapped correctly"
#| fig-scap: "(ref:rerootscap)"
#| fig-show: hold
#| layout-ncol: 2
# reroot
...
#| fig-subcap:
#|   - "Branches scaled in time (year from the root)"
#|   - "Rescaled using dN as branch lengths"
#|   - "Rescaled using substitution rates"
#| fig-scap: "(ref:rescalescap)"
#| out-extra: ''
#| out-width: "100%"
#| fig-show: hold
#| layout-ncol: 3
...
### Removing tips in a phylogenetic tree {#sec-remove-tip}
...
Let's say that we want to remove three tips (colored red) from the tree (Figure @fig-removeTip-1), the `drop.tip()` method removes specified tips and updates the tree (Figure @fig-removeTip-2). All associated data will be maintained in the updated tree. 
...
#| fig-subcap:
#|   - "Original tree with three tips to remove"
#|   - "Updated tree after removing selected tips"
#| fig-scap: "(ref:removeTipscap)"
#| out-width: "100%"
#| fig-show: hold
#| layout-ncol: 2
...
### Subsetting tree by tip label {#sec-subset-tip}

Sometimes a tree can be large and difficult to look at only the portions of interest. The `tree_subset()` function was created in the `r Biocpkg("treeio")` package [@wang_treeio_2020] to extract a subset of the tree portion while still maintaining the structure of the tree portion. The `beast_tree` in Figure @fig-subsetTip-1 is slightly crowded. Obviously, we can make the figure taller to allow more space for the labels (similar to using the "Expansion" slider in `FigTree`) or we can make the text smaller. However, these solutions are not always applicable when you have a lot of tips (*e.g.*, hundreds or thousands of tips). In particular, when you are only interested in the portion of the tree around a particular tip, you certainly don't want to explore a large tree to find the certain species you are interested in.

Let's say you are interested in tip *A/Swine/HK/168/2012* from the tree (Figure @fig-subsetTip-1) and you want to look at the immediate relatives of this tip. 

The `tree_subset()` function allows you to look at the portions of the tree that are of interest. By default, the `tree_subset()` function will internally call the [`groupOTU()`](#groupotu) to assign the group specified tip from the rest of the other tips (Figure @fig-subsetTip-2). Additionally, the branch lengths and related associated data are maintained after subsetting (Figure @fig-subsetTip-3). The root of the tree is always anchored at zero for the subset tree by default and all the distances are relative to this root. If you want all the distances to be relative to the original root, you can specify the root position (by `root.position` parameter) to the root edge of the subset tree, which is the sum of branch lengths from the original root to the root of the subset tree (Figures @fig-subsetTip-4 and @fig-subsetTip-5).
...
#| label: fig-subsetTip
#| fig-cap: |
#|   Subsetting tree for a specific tip.
#| fig-subcap:
#|   - "Original tree"
#|   - "Subset tree"
#|   - "Subset tree with data"
#|   - "Subset tree relative to original position without rootedge"
#|   - "Subset tree relative to original position with rootedge"
#| fig-scap: "(ref:subsetTipscap)"
#| out-extra: ''
#| out-width: "100%"
#| fig-show: hold
#| layout-ncol: 3
...
### Subsetting tree by internal node number {#sec-subset-node}

If you are interested in a certain clade, you can specify the input node as an internal node number. The `tree_subset()` function will take the clade as a whole and also trace it back to particular levels to look at the immediate relatives of the clade (Figures @fig-subsetNode-1 and @fig-subsetNode-2). We can use the `tree_subset()` function to zoom in selected portions and plot a whole tree with the portion of it, which is similar to the `ape::zoom()` function to explore a very large tree (Figures @fig-subsetNode-3 and @fig-subsetNode-4). Users can also use `viewClade()` function to restrict tree visualization at specific clade as demonstrated in [session 6.1](#viewing-selected-clade).
...
#| fig-height: 10
#| echo: true
#| label: fig-subsetNode
#| fig-cap: |
#|   Subsetting tree for the specific clade.
#| fig-subcap:
#|   - "Extract a clade"
#|   - "Extract a clade and trace back to immediate relatives"
#|   - "View a very large tree"
#|   - "View a selected portion of the tree"
#| fig-scap: "(ref:subsetNodescap)"
#| out-extra: ''
#| out-width: "100%"
#| fig-show: hold
#| layout-ncol: 2
...
## Manipulating Tree Data for Visualization {#sec-ggtree-fortify}
...
Here is an example to plot two trees face-to-face that is similar to a graph produced by the `ape::cophyloplot()` function\index{ape} (Figure @fig-cophylo)\index{geom\textunderscore tree}.
...
#| fig-scap: "Plot two phylogenetic trees face-to-face"
#| out-extra: ''
#| out-width: "100%"
library(dplyr)
library(ggtree)
...
It is quite easy to plot multiple trees and connect taxa in one figure; for instance, plotting trees constructed from all internal gene segments of influenza virus and connecting equivalent strains across the trees [@venkatesh_avian_2018]. Figure @fig-cophylo3 demonstrates the usage of plotting multiple trees by combining multiple layers of `geom_tree()`. 
...
#| fig-scap: "Plot multiple phylogenetic trees side-by-side"
#| out-extra: ''
#| out-width: "100%"
z <- rtree(30)
d2 <- fortify(y)
