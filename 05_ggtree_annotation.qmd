...
# Phylogenetic Tree Annotation {#sec-chapter5}
...
Layers defined in `r CRANpkg("ggplot2")` can be applied to `r Biocpkg("ggtree")` directly as demonstrated in Figure @fig-ggtreeNHX of using `geom_label()` and `geom_text()`. But `r CRANpkg("ggplot2")` does not provide graphic layers that are specifically designed for phylogenetic tree annotation. For instance, layers for tip labels, tree branch scale legend, highlight, or labeling clade are all unavailable. To make tree annotation more flexible, several layers have been implemented in `r Biocpkg("ggtree")` (Table @tbl-geoms), enabling different ways of annotation on various parts/components of a phylogenetic tree.
...
The `geom_cladelab()` layer accepts a selected internal node number and labels the corresponding clade automatically (Figure @fig-cladelabel-1). To get the internal node number, please refer to [Chapter 2](#accesor-tidytree). 
...
Users can set the parameter, `align = TRUE`, to align the clade label, `offset`, to adjust the position and color to set the color of the bar and label text, *etc.* (Figure @fig-cladelabel-2). 
...
Users can change the `angle` of the clade label text and relative position from text to bar via the parameter `offset.text`. The size of the bar and text can be changed via the parameters `barsize` and `fontsize`, respectively (Figure @fig-cladelabel-3).
...
Users can also use `geom_label()` to label the text and can set the background color by `fill` parameter (Figure @fig-cladelabel-4).
...
#| fig-subcap:
#|   - "Default"
#|   - "Aligning and coloring clade bar and text"
#|   - "Changing size and angle"
#|   - "Using geom_label() with background color"
#| layout-ncol: 2
#| fig-scap: "(ref:cladelabelscap)"
#| out-width: '100%'
set.seed(2015-12-21)
tree <- rtree(30)
p <- ggtree(tree) + xlim(NA, 8)
...
#| fig-width: 12
#| fig-height: 5
#| label: fig-cladelabaes
#| fig-cap: |
#|   **Labeling clades using aesthetic mapping.**
#| fig-subcap:
#|   - "Use aesthetic mapping to annotate clades"
#|   - "Use images or phylopic to annotate clades"
#|   - "Map variables to change color or size of text or images"
#| fig-scap: "(ref:cladelabaesscap)"
#| out-width: '100%'
#| fig-show: hold
#| layout-ncol: 3
...
The `geom_cladelab()` layer also supports unrooted tree layouts (Figure @fig-striplabel-1).
...
The `geom_cladelab()` is designed for labeling Monophyletic (Clade) while there are related taxa that do not form a clade. In `r Biocpkg("ggtree")`, we provide another layer, `geom_strip()`, to add a strip/bar to indicate the association with an optional label for Polyphyletic or Paraphyletic (Figure @fig-striplabel-2).
...
#| fig-height: 6.5
#| echo: false
#| label: fig-striplabel
#| warning: false
#| fig-cap: |
#|   **Labeling associated taxa.**
#| fig-subcap:
#|   - "geom_cladelab() supports unrooted layouts"
#|   - "geom_strip() labels all types of associated taxa"
#| layout-ncol: 2
#| fig-scap: "(ref:striplabelscap)"
#| out-width: '100%'
...
The `r Biocpkg("ggtree")` implements the `geom_hilight()` layer, which accepts an internal node number and adds a layer of a rectangle to highlight the selected clade (Figure @fig-hilight)^[If you want to plot the tree above the highlighting area, visit [FAQ](#faq-under-the-tree) for details.]\index{geom\textunderscore hilight}.
...
The `geom_hilight` layer also supports highlighting clades for unrooted layout trees with round ('encircle') or rectangular ('rect') shape (Figure @fig-hilight-3).
...
which is designed to highlight neighboring subclades of a given internal node (Figure @fig-hilight-4).
...
The `geom_hilight()` layer supports using aesthetic mapping to automatically highlight clades as demonstrated in Figures @fig-hilight-5 and @fig-hilight-6. For plot in Cartesian coordinates (*e.g.*, rectangular layout), the rectangle can be rounded (Figure @fig-hilight-5) or filled with gradient colors (Figure @fig-hilight-6).
...
#| warning: false
#| fig-cap: |
#|   **Highlight selected clades.**
#| fig-subcap:
#|   - "Rectangular layout"
#|   - "Circular layout"
#|   - "Unrooted layout"
#|   - "Highlight neighboring subclades simultaneously"
#|   - "Highlight clades using external data"
#|   - "Highlight clades using tree-associated data"
#| layout-ncol: 3
#| fig-scap: "(ref:hilightscap)"
#| out-width: '100%'
nwk <- system.file("extdata", "sample.nwk", package="treeio")
tree <- read.tree(nwk)
...
#| label: fig-taxalink
#| fig-cap: |
#|   **Linking related taxa.** This can be used to indicate evolutionary events or relationships between species.
#| fig-subcap:
#|   - "Rectangular layout"
#|   - "Circular layout"
...
#| label: fig-geomRange
#| fig-cap: |
#|   **Displaying uncertainty of evolutionary inference.** The center is anchored to the tree nodes. A second *x*-axis was used for range scaling.
#| fig-subcap:
#|   - "Mean value of the range"
#|   - "Estimated value"
#|   - "Second x-axis for range scaling"
#| layout-ncol: 3
#| fig-scap: "(ref:geomRangescap)"
#| fig-width: 12
#| fig-height: 4
#| echo: false
#| out-width: '100%'
...
#| label: fig-beast
#| fig-cap: |
#|   **Annotating `r pkg_beast` tree with _length\_95%\_HPD_ and posterior.** Branch length credible intervals (95% HPD) were displayed as red horizontal bars and clade posterior values were shown on the middle of branches\index{BEAST}.
#| fig-scap: "Annotating BEAST tree"
#| fig-width: 7
#| out-extra: ''
#| out-width: '100%'
...
#| fig-scap: "Annotating tree with HyPhy ancestral sequences"
#| out-extra: ''
#| out-width: '100%'
...
#| label: fig-codeml
#| fig-cap: |
#|   **Annotating tree with amino acid substitution and *d~N~/d~S~* inferred by `r pkg_codeml`.** Branches were rescaled and colored by *d~N~/d~S~* values, and amino acid substitutions were displayed on the middle of branches.
#| fig-scap: "Annotating tree with CodeML dN/dS"
#| warning: false
#| out-extra: ''
#| fig-height: 4
#| out-width: '100%'
