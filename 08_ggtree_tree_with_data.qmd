...
# Plotting tree with data {#sec-chapter7}
...
## Mapping Data to The tree Structure {#sec-attach-operator}
...
## Aligning Graph to the Tree Based on the Tree Structure {#sec-facet_plot}
...
The `geom_facet()` is designed to work with most of the `geom` layers defined in `r CRANpkg("ggplot2")` and other `r CRANpkg("ggplot2")`-based packages. A list of the geometric layers that work seamlessly with `geom_facet()` and `facet_plot()` can be found in Table @tbl-facet-geom. As the `r CRANpkg("ggplot2")` community keeps expanding and more `geom` layers will be implemented in either `r CRANpkg("ggplot2")` or other extensions, `geom_facet()` and `facet_plot()` will gain more power to present data in the future. Note that different `geom` layers can be combined to present data on the same panel and the combinations of different `geom` layers create the possibility to present more complex data with phylogeny (see also Figures @fig-jv2017 and @fig-gggenes). Users can progressively add multiple panels to present and compare different datasets in the evolutionary context (Figure @fig-plottree). Detailed descriptions can be found in the [supplemental file](https://github.com/GuangchuangYu/plotting_tree_with_data/) of [@yu_two_2018]\index{geom\textunderscore facet}.

```{r}
#| echo: false
#| label: tbl-facet-geom
#| tbl-cap: |
#|   Geometric layers compatible with `geom_facet()` and `facet_plot()`.
facet_geoms <- data.frame(
  Geom = c(
    "geom_point", "geom_col", "geom_bar", "geom_line", "geom_area",
    "geom_boxplot", "geom_violin", "geom_tile", "geom_text", "geom_label"
  ),
  Description = c(
    "Points", "Columns", "Bars", "Lines", "Areas",
    "Boxplots", "Violin plots", "Tiles/heatmaps", "Text labels", "Label boxes"
  ),
  stringsAsFactors = FALSE
)
knitr::kable(facet_geoms, booktabs = TRUE)
```
...
For a timescaled tree, as in this example, it's more common to use *x*-axis by using `theme_tree2`. But with this solution, the heatmap is just another layer and will change the *x*-axis. To overcome this issue, we implemented `scale_x_ggtree()` to set the *x*-axis more reasonably (Figure @fig-gheatmap-2).
...
#| label: fig-gheatmap
#| fig-cap: |
#|   Example of plotting matrix with `gheatmap()`.
#| fig-subcap:
#|   - "H3 influenza tree with a genotype table visualized as a heatmap"
#|   - "Aligned tips with tailored x-axis for divergence times and segments"
#| fig-scap: "(ref:gheatmapscap)"
#| out-width: '90%'
#| fig-show: hold
#| layout-ncol: 1
beast_file <- system.file("examples/MCC_FluA_H3.tree", package="ggtree")
...
### Visualize a tree with multiple associated matrices {#sec-gheatmap-ggnewscale}
...
## Visualize a Tree with Multiple Sequence Alignments {#sec-msaplot}

The `msaplot()` accepts a tree (output of `ggtree()`) and a fasta file, then it can visualize the tree with sequence alignment. We can specify the `width` (relative to the tree) of the alignment and adjust the relative position by `offset`, which is similar to the `gheatmap()` function (Figure @fig-msaplot-1)\index{MSA}.
...
A specific slice of the alignment can also be displayed by specifying the `window` parameter (Figure @fig-msaplot-2)..
...
#| label: fig-msaplot
#| fig-cap: |
#|   Example of plotting multiple sequence alignments with a tree.
#| fig-subcap:
#|   - "Whole MSA sequences visualized with a rectangular tree"
#|   - "Circular tree layout with an alignment window slice"
#| fig-scap: "(ref:msaplotscap)"
#| out-width: '100%'
#| fig-show: hold
#| layout-ncol: 2
library(ggtree)
...
## Composite Plots {#sec-composite_plot}
...
If we align them using `r CRANpkg("cowplot")`, the composite plots are not aligned properly as we anticipated (Figure @fig-composite-1).

```r
cowplot::plot_grid(g, p2, p1, ncol=3) 
```

Using `r CRANpkg("aplot")`, it will do all the dirty work for us and all the subplots are aligned properly as demonstrated in Figure @fig-composite-2.
...
#| label: fig-composite
#| fig-cap: |
#|   Example of aligning tree with data side-by-side to create composite plot.
#| fig-subcap:
#|   - "cowplot() just places subplots together"
#|   - "aplot() reorders tree-associated subplots by tree structure"
#| fig-scap: "(ref:compositescap)"
#| out-width: '100%'
#| fig-show: hold
#| layout-ncol: 1
...
1. Integrating node/edge data to the tree can be mapped to visual characteristics of the tree or other datasets (Figure @fig-attacher).
2. Capable of parsing expressions (math symbols or text formatting), emoji, and image files ([Chapter 8](#chapter8)). 
3. No pre-definition of input data types or how the data should be plotted in `geom_facet()` (Table @tbl-facet-geom).
4. Combining different `geom` functions to visualize associated data is supported (Figure @fig-jv2017).
