...
## Basic R Related {#sec-faq-r}

### Use your local file {#sec-faq-local-file}
...
## Aesthetic mapping {#sec-faq-aes-mapping}

### Inherit aesthetic mapping {#sec-faq-inherit-aes}
...
### Never use `$` in aesthetic mapping {#sec-faq-dollar-aes}
...
## Text and Label {#sec-faq-text}

### Tip label truncated {#sec-faq-label-truncated}
...
In this example, the tip labels displayed in Figure @fig-truncatedTip-1 are truncated. This is because the units are in two different spaces (data and pixel). Users can use `xlim` to allocate more spaces for tip labels (Figure @fig-truncatedTip-2).

```r
p + xlim(0, 0.08)
```

Another solution is to set `clip = "off"` to allow drawing outside of the plot panel. We may also need to set `plot.margin` to allocate more spaces for margin (Figure @fig-truncatedTip-3).
...
#| fig-height: 4
#| echo: false
#| label: fig-truncatedTip
#| fig-cap: |
#|   Allocating more spaces for truncated tip labels.
#| fig-subcap:
#|   - "Long tip labels may be truncated"
#|   - "Allocate more space for plot panel"
#|   - "Allow drawing outside the plot panel"
#| fig-scap: "Allocating more spaces for truncated tip labels"
#| out-width: '100%'
#| fig-show: hold
#| layout-ncol: 3
library(ggplot2)
...
For rectangular/dendrogram layout trees, users can display tip labels as *y*-axis labels. In this case, no matter how long the labels are, they will not be truncated (see Figure @fig-tiplab-3).


### Modify (tip) labels {#sec-faq-modify-label}
...
the modified version (Figure @fig-renameTip).
...
#|   - "Original tip labels"
#|   - "Modified tip labels"
#| fig-scap: "Alternative tip labels"
#| out-width: '100%'
#| fig-show: hold
#| layout-ncol: 2
...
### Formatting (tip) labels {#sec-faq-formatting-label}
...
For example, the tip labels contain several parts (*e.g.*, genus, species, and geo), we can differentiate these pieces of information with different formats (Figure @fig-formatTip-1).
...
Using Markdown or HTML to format text may be easier, and this is supported via the `r CRANpkg("ggtext")` package (Figure @fig-formatTip-2).
...
#|   Formatting labels.
#| fig-subcap:
#|   - "Format tip labels using plotmath expressions"
#|   - "Format tip labels using Markdown/HTML"
#| fig-scap: "Formatting labels"
#| out-width: '100%'
#| fig-show: hold
#| layout-ncol: 2
library(ggtree)
...
species <- c("gorilla", "spp.", "sapiens", "pygmaeus")
geo <- c("Africa", "Africa", "World", "Asia")
d <- data.frame(label = tree$tip.label, genus = genus, species = species, geo = geo)
...
p1 <- ggtree(tree) %<+% d2 + xlim(NA, 6) +
  geom_tiplab(aes(label = lab), parse = TRUE)
...
### Avoid overlapping text labels {#sec-faq-ggrepel}

Users can use the `r CRANpkg("ggrepel")` package to repel overlapping text labels (Figure @fig-repelTip).
...
### Bootstrap values from Newick format {#sec-faq-bootstrap}
...
#| fig-scap: "Bootstrap value stored in node label"
#| out-width: '100%'
...
#| fig-scap: "Ladderized and non-ladderized tree"
#| fig-width: 8
#| fig-height: 4
#| echo: false
#| out-width: "100%"
...
#| fig-scap: "Specifying tree order"
#| label: fig-rotateConstr
#| fig-width: 6
#| fig-height: 4
#| out-width: "100%"
...
When outgroups are on a very long branch length (Figure @fig-outgroupEdge-1), we would like to keep the outgroups in the tree but ignore their branch lengths (Figure @fig-outgroupEdge-2)^[Example from: <https://groups.google.com/d/msg/bioc-ggtree/T2ySvqv351g/mHsyljvBCwAJ>]. This can be easily done by modifying the coordinates of the outgroups (Figure @fig-outgroupEdge-2). Another approach is to truncate the plot using the `r CRANpkg("ggbreak")` package (Figure @fig-outgroupEdge-3) [@ggbreak].
...
#| fig-cap: |
#|   Shrink outlier long branch.
#| label: fig-outgroupEdge
#| fig-subcap:
#|   - "Original tree"
#|   - "Reduced outgroup branch length"
#|   - "Truncated tree plot"
#| fig-scap: "Shrink outlier long branch"
#| fig-width: 9
#| fig-height: 5
#| out-width: "100%"
#| fig-keep: "all"
...
### Attach a new tip to a tree {#sec-bind-tip}
...
Users can use `phytools::bind.tip()` [@revell_phytools_2012] to attach a new tip to a tree. With `r CRANpkg("tidytree")`, it is easy to add an annotation to differentiate newly introduced and original branches and to reflect the uncertainty of the added branch splits off, as demonstrated in Figure @fig-bindTip. 
...
#| fig-scap: "Attaching a new tip to a tree"
#| fig-width: 6
#| fig-height: 5
#| out-width: "100%"
library(phytools)
...
#| fig-scap: "Change colors and line types of specific branches"
#| fig-width: 7
#| fig-height: 5
#| out-width: "100%"
set.seed(123)
...
```{r}
#| fig-cap: |
#|   Add an arbitrary point on a branch.
#| label: fig-pointOnBranch
#| fig-subcap:
#|   - "Adjust symbolic point by an absolute offset"
#|   - "Adjust symbolic point by branch length proportion"
#| fig-scap: "Add an arbitrary point on a branch"
#| fig-width: 8
#| fig-height: 4.5
#| out-width: "100%"
#| fig-show: hold
...
This is not supported by `r CRANpkg("ggplot2")` in general. However, we can just draw text labels for each panel and put the labels beyond the plot panels as demonstrated in Figure @fig-xlabFacets.
...
#| fig-width: 7
#| fig-height: 5
#| label: fig-xlabFacets
#| fig-cap: |
#|   ***X*-axis titles for different facet panels.**
#| fig-scap: "X-axis titles for different facet panels"
#| out-width: "100%"
library(ggtree)
library(ggplot2)
set.seed(2019-05-02)
...
## Plot Something behind the Phylogeny {#sec-faq-under-the-tree}
...
#| fig-width: 6
#| fig-height: 3
#| label: fig-treeLayerOrder
#| fig-cap: |
#|   **Add layers behind the tree structure.**
#| fig-subcap:
#|   - "Layer on top of the tree structure"
#|   - "Reverse the layer order"
#|   - "Add layer behind the tree layer"
#| fig-scap: "(ref:treeLayerOrderscap)"
#| out-width: "100%"
#| fig-show: hold
#| layout-ncol: 3
g <- p
...
## Enlarge Center Space in Circular/Fan Layout Tree {#sec-faq-enlarge-center-space}

This question for enlarging center space in circular/fan layout tree was asked several times^[<https://groups.google.com/d/msg/bioc-ggtree/gruC4FztU8I/mwavqWCXAQAJ>, <https://groups.google.com/d/msg/bioc-ggtree/UoGQekWHIvw/ZswUUZKSGwAJ> and <https://github.com/GuangchuangYu/ggtree/issues/95>], and a published example can be found in [@barton_broad_2016]. Increasing the percentage of center white space in a circular tree is useful to avoid overlapping tip labels and to increase the readability of the tree by moving all nodes and branches further out. This can be done simply by using `xlim()` or `hexpand()` to allocate more space (Figure @fig-circular-space-1), just like in Figure @fig-layout2-7, or assigning a long root branch that is similar to the "Root Length" parameter in `r pkg_figtree` (Figure @fig-circular-space-2). 
...
#| fig-height: 4
#| fig-cap: |
#|   Enlarge center space in circular tree.
#| label: fig-circular-space
#| fig-subcap:
#|   - "Allocate more space by xlim"
#|   - "Allocate more space by adding a long root branch"
#| fig-scap: "Enlarge center space in circular/fan layout tree"
#| out-width: "100%"
#| fig-show: hold
#| layout-ncol: 2
set.seed(1982)
...
#| label: fig-distantTip
#| fig-subcap:
#|   - "Forward: from the root to the tips"
#|   - "Backward: from the most distant tip to the root"
#| fig-scap: "Plot distance between tips"
#| fig-width: 6
#| fig-height: 3
#| out-width: "100%"
#| fig-show: hold
...
## Remove Blank Margins for Circular Layout Tree {#sec-circular-blank}
...
#| fig-height: 4.5
#| fig-cap: |
#|   **Trim extra white space for polar coordinates.**
#| label: fig-trimSpace
#| fig-subcap:
#|   - "Original plot"
#|   - "Trimmed version"
#| fig-scap: "Trim white space in circular layout tree"
#| out-width: "100%"
#| fig-show: hold
#| layout-ncol: 2
library(ggplot2)
...
## Edit Tree Graphic Details {#sec-export-edit}
