...
# ggtree Utilities {#sec-chapter12}
...
## Facet Utilities {#sec-facet-utils}

### facet_widths {#sec-facet_widths}
...
It also supports using a name vector to set the widths of specific panels. The following code will display an identical figure to Figure @fig-facetWidth-1.
...
The `facet_widths()` function also works with other `ggplot` objects as demonstrated in Figure @fig-facetWidth-2.
...
#| fig-width: 6
#| fig-height: 7
#| label: fig-facetWidth
#| fig-scap: "(ref:facetWidthscap)"
#| fig-cap: |
#|   Adjust relative widths of ggplot facets.
#| fig-subcap:
#|   - "facet_widths() applied to ggtree"
#|   - "facet_widths() applied to ggplot"
#| fig-show: hold
#| layout-ncol: 1
library(ggplot2)
library(ggtree)
...
### facet_labeller {#sec-facet_labeller}

The `facet_labeller()` function was designed to relabel selected panels (Figure @fig-facetLab), and it currently only works with `ggtree` objects (*i.e.*, `geom_facet()` outputs). A more versatile version that works with both `ggtree` and `ggplot` objects is implemented in the `r CRANpkg("ggfun")` package (*i.e.*, the `facet_set()` function).
...
#| fig-subcap:
#|   - "Rename multiple labels simultaneously"
#|   - "Rename a specific label and adjust relative widths"
#| fig-show: hold
#| layout-ncol: 1
...
These layers works with both `r Biocpkg("ggtree")` and `r CRANpkg("ggplot2")` (Figure @fig-layer2).
...
#| label: fig-layer2
#| fig-cap: |
#|   **Geometric layers that support subsetting.**
#| fig-subcap:
#|   - "These layers work with ggplot2"
#|   - "These layers work with ggtree"
#| fig-scap: "(ref:layer2scap)"
#| out-width: "100%"
#| fig-show: hold
#| layout-ncol: 2
library(ggplot2)
...
In [session 4.2](#tree-layouts), we introduce several layouts supported by `r Biocpkg("ggtree")`. The `r Biocpkg("ggtree")` package also provides several layout functions that can transform from one to another. Note that not all layouts are supported (see Table @tbl-layoutLayerTab and Figure @fig-layoutLayer)\index{tree layout}.

```{r}
#| echo: false
#| label: tbl-layoutLayerTab
#| tbl-cap: |
#|   Layout transformers.
...
knitr::kable(layout.df, booktabs = TRUE)
```
...
#| fig-height: 7.5
#| message: false
#| label: fig-layoutLayer
#| fig-cap: |
#|   **Layout functions for transforming among different layouts**.
#| fig-subcap:
#|   - "Default rectangular layout"
#|   - "Transform rectangular to dendrogram layout"
#|   - "Transform circular to rectangular layout"
#|   - "Transform rectangular to circular layout"
#|   - "Transform rectangular to fan layout"
#|   - "Transform rectangular to inward circular layout"
#| fig-scap: "(ref:layoutLayerscap)"
#| out-width: '100%'
#| fig-show: hold
#| layout-ncol: 3
set.seed(2019)
...
The `xlim_expand()` function also works with `ggplot2::facet_grid()`. As demonstrated in Figure @fig-xlimExpand-2, only the `xlim` of *virginica* panel was adjusted by `xlim_expand()`.
...
```{r}
#| echo: false
#| label: fig-xlimExpand
#| fig-cap: |
#|   Setting xlim for user-specific facet panels.
#| fig-subcap:
#|   - "xlim_expand() applied to ggtree facets"
#|   - "xlim_expand() applied to ggplot facets"
#| fig-scap: "(ref:xlimExpandscap)"
#| fig-width: 12
#| fig-height: 5
#| out-width: '100%'
#| fig-show: hold
...
### Expand plot limit by the ratio of plot range {#sec-ggexpand}
...
#| label: fig-hexpand
#| fig-scap: "(ref:hexpandscap)"
#| fig-width: 12
#| fig-height: 5
#| out-width: '100%'
#| fig-subcap:
#|   - "Expand x limit at the right-hand side"
#|   - "Expand x/y limits with direction = -1"
#| fig-show: hold
#| layout-ncol: 2
...
### Filter tree data {#sec-td_filter}
...
#| fig-scap: "Filtering ggtree plot data"
#| fig-width: 7
#| fig-height: 5
library(tidytree)
...
### Flatten list-column tree data {#sec-td_unnest}
...
#| label: fig-tdUnnest
#| fig-cap: |
#|   Flattening ggtree plot data.
#| fig-subcap:
#|   - "Flatten list-columns with td_unnest()"
#|   - "Combine td_filter() with td_unnest()"
#| fig-scap: "Flattening ggtree plot data"
#| fig-width: 8
#| fig-height: 5
#| out-width: '100%'
#| fig-show: hold
...
### Extract tip order {#sec-tiporder}
...
## Interactive ggtree Annotation {#sec-identify}
...
+ Highlighting clades: [Youtube](https://youtu.be/KcF8Ec38mzI) and [Youku](http://v.youku.com/v_show/id_XMTYyMzgyODYyOA).
+ Labelling  clades: [Youtube](https://youtu.be/SmcceRD_jxg) and [Youku](http://v.youku.com/v_show/id_XMTYyNDIzODA0NA). 
+ Rotating clades: [Youtube](https://youtu.be/lKNn4QlPO0E) and [Youku](http://v.youku.com/v_show/id_XMTYyMzgyODg2OA). 
