...
# Annotating Tree with Silhouette Images and Sub-plots {#sec-chapter8}
...
## Annotating Tree with Images {#sec-ggimage}

We usually use text to label taxa, *i.e.* displaying taxa names. If the text is the image file name (either local or remote), `r Biocpkg("ggtree")` can read the image and display the actual image as the label of the taxa (Figure @fig-frogimage). The `geom_tiplab()` and `geom_nodelab()` are capable to render silhouette images with supports from in-house developed package, `r CRANpkg("ggimage")`\index{ggimage}. 
...
## Annotating Tree with Sub-plots {#sec-subplots}
...
This example uses `ape::ace()` function to estimate ancestral character states. The likelihoods of the stats were visualized as stacked bar charts which were overlayed onto internal nodes of the tree using the `geom_inset()` layer (Figure @fig-pieInset-1)\index{geom\textunderscore inset}. 
...
Similarly, users can use the `nodepie()` function to generate a list of pie charts and place these charts to annotate corresponding nodes (Figure @fig-pieInset-2). Both `nodebar()` and `nodepie()` accept a parameter of `alpha` to allow transparency. 
...
#| label: fig-pieInset
#| fig-cap: |
#|   Annotate internal nodes with bar or pie charts.
#| fig-subcap:
#|   - "Using bar charts to display summary statistics of internal nodes"
#|   - "Using pie charts to display summary statistics of internal nodes"
#| fig-scap: "(ref:pieInsetscap)"
#| out-width: '100%'
#| fig-show: hold
#| layout-ncol: 2
...
Parsing clade labels as emojis is also supported in the `geom_cladelab()` layer. For example, in a phylogenetic tree of influenza viruses, we can use emojis to label clades to represent host species similar to Figure @fig-emojiclade\index{emojifont}. 
...
#| fig-width: 6
#| fig-height: 5
#| label: fig-emojiclade
#| fig-cap: |
#|   **Emoji to label clades.**
#| fig-scap: "(ref:emojicladescap)"
#| out-width: '100%'
set.seed(123)
tr <- rtree(30)
