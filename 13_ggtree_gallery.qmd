...
# Gallery of Reproducible Examples {#sec-chapter13}




## Visualizing pairwise nucleotide sequence distance with a phylogenetic tree {#sec-hpv58}
...
## Displaying Different Symbolic Points for Bootstrap Values. {#sec-symbolic-bootstrap}
...
## Highlighting Different Groups {#sec-phylo-grouping}
...
#| fig-scap: "(ref:treeLarsenscap)"
#| label: fig-treeLarsen
#| fig-width: 7.5
#| fig-height: 6.3
#| out-width: '100%'
library(TDbook)
...
## Phylogenetic Tree with Genome Locus Structure {#sec-genome-locus}

The `geom_motif()` is defined in `r Biocpkg("ggtree")` and it is a wrapper layer of the `gggenes::geom_gene_arrow()`. The `geom_motif()` can automatically adjust genomic alignment by selective gene (via the `on` parameter) and can label genes via the `label` parameter. In the following example, we use `example_genes` dataset provided by `r CRANpkg("gggenes")`. As the dataset only provides genomic coordination of a set of genes, a phylogeny for the genomes needs to be constructed first. We calculate Jaccard similarity based on the ratio of overlapping genes among genomes and correspondingly determine genome distance. The BioNJ algorithm was applied to construct the tree. Then we can use `geom_facet()` to visualize the tree with the genomic structures (Figure @fig-gggenes). 
